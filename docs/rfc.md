...
iOS app 和 FastAPI 之间通过 Tailscale 内网通信。不需要公网暴露，不需要认证。FastAPI 默认监听 `0.0.0.0:7980`。

```
iPhone (Tailscale) --POST--> Mac (Tailscale:7980) --write--> SQLite
                                                         --read--> CLI
```
...
CLI 只负责 readout / analytics，不碰 ingestion：
