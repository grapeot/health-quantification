...
6. 日级 summary contract 与简单 daily artifact
...
AI 是第一优先级用户。它需要稳定的命令入口、可预测的 JSON 输出、清晰的数据边界，以及默认不会碰到真实敏感数据的测试策略。AI 应该能在不读散装脚本的前提下，完成配置检查、数据库初始化、summary 查询、artifact 生成，以及通过 Swagger UI 自助理解 ingestion API。
...
- `health_quantification.artifacts.chart`：简单 daily card artifact 输出
- `health_quantification.cli`：薄 CLI（analytics only，不碰 ingestion）
...
- 支持配置 server URL（默认 `http://quantum.tail63c3c5.ts.net:7996`）
- ATS exception 白名单 `quantum.tail63c3c5.ts.net`（自定义 Info.plist）
- Unit tests、UI tests
